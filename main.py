...
def gconnect():
    """
        This fucntion is responsible for gathering all the info from
         the Google account used to logginIn/sigingIn
        """
    ...
    try:
        oauth_flow = flow_from_clientsecrets(
            'client_secrets.json', scope='', redirect_uri='')
    ...
    if result['user_id'] != ggplus_id:
        response = make_response(json.dumps(
            "Token's user ID doesnt match given user ID"), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
